...
bool Game::hit(QPoint pos)
{
    ...
    if (r1.intersects(mouse)){qDebug()<<"intersect with 1 item";}
    if (r2.intersects(mouse)){qDebug()<<"intersect with 2 item";}
    if (r3.intersects(mouse)){qDebug()<<"intersect with 3 item";}
}
...
QPixmap *Game::getScene(GameTask *task)
{
    ...
    gamePainter->create();
    gamePainter->draw(0,0,getResImg(":/img/testRoom.png"));
